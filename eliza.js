...
ElizaBot.prototype.transform = function(text) {
	...
	return (rpl!='')? rpl : 'Can you say in another word?';
}
...
ElizaBot.prototype._execRule = function(k) {
	...
	for (var i=0; i<decomps.length; i++) {
		...
		if (m!=null) {
			...
			var logo=decomps[i][2]
			var memflag=decomps[i][3];
			...
			if (m1) {
				...
				while (m1) {
					...
					if (m2) {
						...
						var lg2=param;
						while (m2) {
							lp2+=lg2.substring(0,m2.index)+this.posts[m2[1]];
							lg2=lg2.substring(m2.index+m2[0].length);
							m2=this.postExp.exec(lg2);
						}
						param=lp2+lg2;
					}
					lp+=lg.substring(0,m1.index)+param;
					lg=lg.substring(m1.index+m1[0].length);
					m1=paramre.exec(lg);
				}
				logo=lp+lg;
			}
			logo=this._postTransform(logo);


			if (memflag) this._memSave(rpl)
			else return [rpl,logo];
		}
	}
	return '';
}
